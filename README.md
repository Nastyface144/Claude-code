...
|---|---|---|---|
| **GitHub Actions + внешний планировщик** | нет | ~30 мин, точно по расписанию | **рекомендуется**: гарантированно бесплатно, репозиторий остаётся приватным |
...
### Вариант 1. GitHub Actions + бесплатный внешний планировщик — рекомендуется

Полностью бесплатно, без карты, репозиторий может оставаться **приватным**. Идея: GitHub
Actions выполняет опрос и рассылку, а точным временем запуска управляет внешний
бесплатный сервис — расписание самого GitHub слишком приблизительное (наблюдалось:
первый запуск после включения — только через час, дальше — не каждые 5 минут, а
раз в 10–60 минут, слоты пропускаются). Внешний планировщик этого не делает.
...
2. **Settings → Developer settings → Personal access tokens → Fine-grained tokens →
   Generate new token.** Дать доступ только к этому репозиторию, разрешение
   **Contents: Read and write** (нужно, чтобы дёргать workflow) — этот токен пойдёт
   не в GitHub, а в планировщик на шаге 3.
3. Зарегистрироваться на **cron-job.org** (бесплатно, без карты) и создать задание:
   ...
   - **Периодичность:** каждые 30 минут (см. расчёт лимита ниже)
4. Написать своему боту **/start**.

Дальше ничего делать не нужно: cron-job.org будит workflow точно по расписанию, тот
опрашивает биржи и шлёт заказы, а сама GitHub-схема (`0 */6 * * *`) остаётся резервной
на случай, если внешний планировщик остановится.

**Почему именно 30 минут, а не чаще.** Бесплатный план GitHub Actions даёт 2000 минут
в месяц на приватный репозиторий, и каждый запуск округляется вверх до целой минуты.
При интервале 30 минут это ~1440 запусков в месяц — то есть около 1440 минут, с запасом
до лимита. При интервале 5–10 минут лимит будет исчерпан раньше конца месяца, и бот
замолчит до следующего месяца. Если готовы сделать репозиторий публичным — лимита минут
нет вообще, тогда интервал можно снизить хоть до 5 минут (см. вариант ниже).

Полезно знать:

- команды (`/score`, `/add`, `/sources`) отвечают с той же задержкой, что и рассылка, —
  в момент следующего срабатывания планировщика;
...
- если позже расхочется третьего сервиса (cron-job.org) — можно вернуться к
  расписанию самого GitHub, просто будет медленнее и не так предсказуемо.

#### Если репозиторий можно сделать публичным

Тогда лимита минут нет вовсе, и можно опрашивать чаще без внешнего планировщика —
достаточно родного `cron` в `radar.yml` (например, `*/5 * * * *`). Учтите: ветка
`radar-state` (создаётся вариантом ниже с хранением в ветке) хранит id чатов —
для публичного репозитория состояние уже перенесено в кэш Actions, так что это
больше не проблема.
