...
|---|---|---|---|
| **GitHub Actions по расписанию** | нет | от 5 минут до часа | простой старт без сервера |
| **Бесплатный хостинг контейнеров** | нет | мгновенно на команды | когда компьютера под рукой нет |
| **Свой компьютер / VPS** | да | мгновенно на команды, биржи раз в 10 мин | когда нужен предсказуемый бот |
...
GitHub сам поднимает окружение, разбирает накопившиеся команды, опрашивает биржи и
рассылает новые заказы. Состояние (что уже отправлено) хранится в кэше Actions —
не в ветке, потому что в базе лежат id чатов, а репозиторий может быть публичным.
...
Точный запуск без своего компьютера: workflow слушает событие `repository_dispatch`,
поэтому его можно дёргать внешним бесплатным планировщиком (cron-job.org и подобные) —
он не пропускает слоты, в отличие от расписания GitHub:

```
POST https://api.github.com/repos/<владелец>/<репозиторий>/dispatches
Authorization: Bearer <личный токен GitHub с правом repo>
Content-Type: application/json

{"event_type": "radar"}
```

Важно про частоту: `cron` в GitHub Actions — это пожелание, а не гарантия. На бесплатном
плане запуски пропускаются: при настройке «каждые 5 минут» реально наблюдалось около
одного запуска за 10–60 минут, а самый первый пришёл через час после включения. Если
нужен предсказуемый бот — вариант 2.

Полезно знать:

- команды (`/score`, `/add`, `/sources`) в этом режиме тоже работают, но ответ придёт
  в течение 5 минут — бот разбирает накопившиеся сообщения в начале каждого запуска;
- **репозиторий должен быть публичным**: в приватном Actions расходуют бесплатные
  2000 минут в месяц, и запуск раз в 5 минут в них не влезает (каждый запуск
  округляется до минуты). Для приватного ставьте в `radar.yml` `*/30`;
- сами биржи опрашиваются не чаще раза в 10 минут (`MIN_SOURCE_INTERVAL`), даже если
  бот просыпается каждые 5, — чтобы не собирать блокировки;
